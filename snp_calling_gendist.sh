...
#
# Usage:
#   snp_calling_gendist.sh <ref.fa> <indir> <outdir>
#
# Description:
#   Call and filter SNPs with ANGSD. Uses giraffe and okapi BAMs.
#
# Requirements:
#   angsd
#   python3
...
# find all BAMs including giraffe and the okapi
bams=$(find $2 -name '*.clean.bam' -printf '%p ')

# calculate global site depth
sambamba depth base -t 8 --combined ${bams} | awk 'NR>1 { print $3 }' > $3/site.depth

# calculate site depth statistics
python3 ./site_depth_stats.py $3/site.depth > $3/site.depth.stats

# create an array of population names
pops=(WA GNP ZNP SNR ETH MF RET ISC LVNP MA SGR MTNP BNP SUN KKR V23 ENP HNB WOAK)
...
min_dp=$(grep -Po '\(MEDIAN - 5 \* MAD\): \K-*\d+' $3/site.depth.stats)
max_dp=$(grep -Po '\(MEDIAN \+ 5 \* MAD\): \K\d+' $3/site.depth.stats)
